...
export function NotesByDatePage({ compact }: { compact?: boolean }) {
  const { dateKey = '' } = useParams()
  const { notesOnDate } = useNotes()
  const navigate = useNavigate()
  const notes = notesOnDate(dateKey)
  const label = dateKey.split('-').join('/')
    ...
    <div className={cn('px-4 pt-4', compact ? 'pt-8' : 'lg:px-8 lg:pt-8')}>
      <header className="mb-4 flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-xs font-semibold text-sn-muted">
            {notes.length} NOTE
          </p>
        </div>
      </header>
